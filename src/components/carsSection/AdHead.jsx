    ...
    return (
        <div className={`w-full h-[20.5rem] bg-blue-400 rounded-xl relative transition-all overflow-hidden text-white`} id={adData?.id} onClick={() => dispatch(openBannerAndFetch(adData?.id))} >
            <div className={` relative z-10 p-7`}>
                <h2 className={`text-3xl font-bold `}>{adData?.heading}</h2>
                <p className={`  w-6/12`}>{adData?.desc?.slice(0,100)}</p>
            ...
            <img src={adData?.img} alt="adcar" className={`h-56 object-cover absolute bottom-5 left-44 z-[2] `} />
