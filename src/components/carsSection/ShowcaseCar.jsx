            ...
            <div className='carDiv h-full flex gap-5'>
                <div className='carImgDiv w-1/2 h-full flex flex-col gap-3 overflow-hidden relative group '>
                    ...
                    <div className='flex gap-3 h-[8rem]'>
                        <div className='flex-1 h-full overflow-hidden rounded-xl relative '>
                            ...
                            <img src={carDetails?.media?.backImage} alt="" className='w-full scale-90 object-fit z-10 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' />
                        </div>
                        <div className='flex-1 h-full overflow-hidden rounded-xl relative'>
                            <img src={carDetails?.media?.topImage} alt="" className='w-full  h-full object-fit' />
                        </div>
                        <div className='flex-1 h-full overflow-hidden rounded-xl relative bg-white'>
                            <img src={carDetails?.media?.sideImage} alt="" className='w-full  h-full object-fit' />
                        </div>
                    </div>
                </div>
                ...
                <div className='detailsCard w-1/2 h-full bg-white px-7 py-7 rounded-xl'>
                    <div className='flex justify-between items-center'>
                        <h2 className='font-bold text-3xl'>{carDetails?.basicInfo?.carName} - ({carDetails?.basicInfo?.brand})</h2>
                        {
                    ...
                    <h2 className='text-secondary-300 text-sm mt-5 text-justify leading-6'>{carDetails?.info?.description}</h2>
                    ...
                    <div className='flex justify-between items-center mt-8 transition-all duration-100'>
                        <div className=''>
                            <h2 className='font-bold text-2xl text-secondary-500'>₹ {carDetails?.pricing?.price.toFixed(2)} / <span className='text-sm text-secondary-300 inline-block'>Day</span></h2>
                            <h2 className='font-semibold text-xl text-secondary-300 line-through'>₹ {((carDetails?.pricing?.price / (1 - ( carDetails?.pricing?.discount / 100))))?.toFixed(2)}</h2>
